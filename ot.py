...
def frame_reader_thread_func(cap, frame_input_queue, stop_event, target_width=None):
    ...
    if target_width and target_width > 0 and target_width < original_width : 
        ...
        print(f"[INFO] Frame reader: Processing at original resolution {original_width}x{original_height}")
...
def main():
    ...
    # This is a global setting for PyTorch.
    try:
        cpu_cores = os.cpu_count()
        if cpu_cores:
            # Using half the cores, ensuring at least 1.
            # You can experiment with cpu_cores directly or other values.
    ...
    if target_processing_width and target_processing_width > 0 and target_processing_width < original_width:
        ...
        aspect_ratio = original_height / original_width
        processing_width = effective_target_width_for_reader
        processing_height = int(processing_width * aspect_ratio)
        if processing_height % 2 != 0: processing_height +=1 
        print(f"[INFO] Target processing width set to {processing_width}. Output video will be {processing_width}x{processing_height}")
    else:
        print(f"[INFO] Processing at original resolution: {original_width}x{original_height}")
    ...
    temp_out_silent_video_writer=cv2.VideoWriter(temp_video_file_abs_path, fourcc,float(fps),(processing_width,processing_height))
    ...
    print(f"[INFO] Temp silent video will be {processing_width}x{processing_height} at {temp_video_file_abs_path}")
    ...
    if FFMPEG_AVAILABLE:
        ...
        # Call process_audio_ffmpeg without compatibility_mode, as it's now hardcoded inside
